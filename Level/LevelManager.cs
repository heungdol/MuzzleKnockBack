...
public class LevelManager : MonoBehaviour {
	...
	void Start () {
		...
		currentLevel.GetComponent<LevelInfo> ().enabled = true;
		playerController.transform.position = currentLevel.GetComponent<LevelInfo> ().GetPlayerStartPosition ();
		SetNextLevel();
		nextLevel.transform.position = nextLevelPivot.transform.position;
		nextLevel.GetComponent<LevelInfo> ().enabled = false;
		numberOfPassedLevel++;
	}
	...
	IEnumerator ChangeLevel () {
		...
		currentLevel.GetComponent<LevelInfo> ().enabled = false;
		nextLevel.transform.position = nextLevelPivot.transform.position;
		...
		Vector3 playerTargetPosition = currentLevel.transform.position + (nextLevel.GetComponent<LevelInfo> ().GetPlayerStartPosition () - nextLevelPivot.transform.position);
	...
	public void SetCurrentLevelComponentOff () {
